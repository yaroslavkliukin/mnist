...
def load_train(cfg, save_path="./data"):
    subprocess.run(["dvc", "pull", f"{cfg.data.train_data_file}.dvc"])
    subprocess.run(["dvc", "pull", f"{cfg.data.train_labels_file}.dvc"])
...
def load_test(cfg, save_path="./data"):
    subprocess.run(["dvc", "pull", f"{cfg.data.test_data_file}.dvc"])
    subprocess.run(["dvc", "pull", f"{cfg.data.test_labels_file}.dvc"])
